<script>
import { getCar, addRent } from '../api.js';
...
export default {
  ...
  computed: { getRentTime() { let diff = this.getDateDiff(this.rent_start, this.rent_end);
      switch (diff) {
        case 0:
          return '0 days';
        case 1:
  ...
  methods: {
    ...
    async rentCar() {
      ...
      if (!this.car.rentable) return this.$router.push('/cars');
      await addRent({
	...
	>
		...
		<div class="flex flex-col items-center justify-center">
			<div class="flex flex-col py-4 border-b border-b-zinc-400">
				<h2 class="text-2xl font-bold text-center py-4">Rental time</h2>
				<div
				...
				>
					...
					>
						<label for="rent_start" class="text-xl">Start</label>
						<input
							id="rent_start"
							v-model="rent_start"
							type="datetime-local"
					...
					<div
						class="flex flex-col sm:flex-row gap-2 justify-between items-center w-full"
					>
						<label for="rent_end" class="text-xl">End</label>
						<input
							id="rent_end"
							v-model="rent_end"
							type="datetime-local"
				...
				<h2 class="text-2xl font-bold text-center my-4">Deposit</h2>
				<div class="accent-red-500">
					<div class="flex flex-row items-center gap-2">
						<input id="deposit" v-model="deposit" type="radio" :value="true" />
						<label for="deposit" class="text-lg">With Deposit</label>
					</div>
					<div class="flex flex-row items-center gap-2">
						<input
							id="no_deposit"
							...
							:value="false"
						/>
						<label for="no_deposit" class="text-lg">Without Deposit</label>
					</div>
				</div>
				<h2 class="text-2xl font-bold text-center my-4">Payment</h2>
				<div class="accent-red-500">
					<div class="flex flex-row items-center gap-2">
						<input
							id="payment_card"
							...
							value="card"
						/>
						<label for="payment_card" class="text-lg">With a Credit Card</label>
					</div>
					<div class="flex flex-row items-center gap-2">
						<input
							id="payment_money"
							...
							value="money"
						/>
						<label for="payment_money" class="text-lg">With Cash</label>
					</div>
				</div>
			</div>
			<div class="flex flex-col items-center justify-center">
				<h2 class="text-2xl font-bold text-center my-4">Summary</h2>
				<ul class="flex flex-col items-center justify-center gap-2">
					...
					/>
					<RentSummaryItem text="Rent time" :value="getRentTime" />
					<RentSummaryItem
						text="Payment"
						:value="payment_type === 'card' ? 'Credit Card' : 'Cash'"
					/>
					<RentSummaryItem
						text="Deposit"
						:value="`${deposit ? `Yes` : 'No'}`"
					/>
			...
			<div class="flex flex-col sm:flex-row gap-4 mt-6 w-full max-w-screen-sm">
				<RouterLink :to="`/cars/${car.id_car}`" class="w-full">
					<ButtonSecondary width="full">Return</ButtonSecondary>
				</RouterLink>
				<ButtonPrimary width="full" @click="rentCar">Rent</ButtonPrimary>
			</div>
		</div>
	</main>
	<div
